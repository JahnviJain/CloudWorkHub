...
export class CloudWorkHub extends Room<OfficeState> {
  ...
  async onAuth(client: Client, options: { password: string | null }) {
    if (this.password && options.password !== null) { // Check if options.password is not null
      const validPassword = await bcrypt.compare(options.password, this.password)
      if (!validPassword) {
        throw new ServerError(403, 'Password is incorrect!')
      }
    } else {
      throw new ServerError(400, 'Password is required!')
    }
    return true
}
